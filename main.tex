...
% Abstract
\abstract{The rapid growth of internet hospitals has created an urgent need for large language model (LLM)-based clinical assistants that can generate personalized, physician-specific responses during remote consultations. Existing medical QA systems produce generic outputs that fail to capture individual doctors' linguistic styles, departmental norms, and institutional constraints, leading to low physician adoption. We propose a three-stage personalized adaptation framework that addresses this gap through: (1) doctor-specific Low-Rank Adaptation (LoRA) training via the weclone pipeline; (2) LoRA-Augmented Generation (LAG) routing for dynamic adapter selection; and (3) TIES-MERGING, an interference-aware model fusion technique that preserves 97.1\% of individual adapter performance after merging. We construct the Internet Hospital Doctor-Patient Dialogue (IHDPD) dataset spanning 15 clinical departments and 120 physicians. Extensive evaluations demonstrate significant improvements over competitive baselines, with practicing physicians consistently preferring our personalized outputs. The framework offers a practical pathway toward trustworthy, physician-centered AI assistance in clinical workflows.}
...
\Title{WebDoctor: Doctor Response Assistant via LoRA Routing and Merging in Internet Hospital Services}
...
\Author{Yubo Wang $^{1}$\orcidA{}, Firstname Lastname $^{2}$ and Firstname Lastname $^{2,}$*}
