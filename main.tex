...
\abstract{The rapid growth of internet hospitals has created an urgent need for LLM-based clinical assistants that generate physician-specific responses during remote consultations. Existing medical QA systems produce generic outputs that fail to capture individual doctors' linguistic styles, departmental norms, and institutional constraints, limiting physician adoption. We propose a three-stage personalized adaptation framework. First, doctor-specific Low-Rank Adaptation (LoRA) modules are trained via the weclone pipeline on each physician's consultation records, producing compact adapters that encode individual response patterns. Second, LoRA-Augmented Generation (LAG) routing dynamically selects the appropriate adapter at inference time through identity-based or query-based retrieval. Third, TIES-MERGING, an interference-aware model fusion technique, consolidates multiple adapters into a unified deployment artifact while preserving 97.1\% of individual adapter performance. We construct the Internet Hospital Doctor-Patient Dialogue (IHDPD) dataset spanning 15 clinical departments and 120 physicians. Evaluations with automatic metrics and board-certified physician raters show significant improvements over baselines: the framework achieves a BLEU-4 of 10.4 and personalization rating of 4.2/5.0, substantially above the base model (BLEU-4: 5.1, personalization: 2.1). The framework offers a practical pathway toward trustworthy, physician-centered AI assistance in clinical workflows.}
